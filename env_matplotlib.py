...
class EnvRender(EnvMarsLander):
    ...
    def step(self,action):
        ...
        if done:
            plt.show()
            print(f"Number of steps : {self.number_state}")
            print()
        return done
...
evolution_number = 400
population_size = 80
gene_size = 100
def main():
    env = EnvMarsLander(test_input[0],test_input[1])
    env_render = EnvRender(test_input[0],test_input[1])
    env.reset()
    env_render.reset()
