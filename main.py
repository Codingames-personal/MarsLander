...
NONGRADED_RETAIN_PERCENT = 0.5  # percentage of retained remaining individuals (randomly selected)
MUTATION_PROBABILITY = 0.01

sigmoid = lambda x : 2 - 2/(1 + math.exp(-x)) 
...
class EnvMarsLander:
    ...
    def get_score_landing(self) -> float:
        ...
        WEIGHT_DISTANCE = 3000
        if self.landing_on_site():
            distance_score = 1
        elif not (0 <= self.lander.x < 7000):
            distance_score = -1
        else:
            distance_score = sigmoid(self.point_lander_now.distance(self.landing_site_point)/WEIGHT_DISTANCE)
    ...
    def step(self,action : Action) -> bool:
        ...
        if self.surface.collision(self.point_lander_before, self.point_lander_now):
            self.previous_lander = self.lander
            return "last_step"
...
class Chromosome:
    ...
    def use(self,env):
        ...
        self.score = -1e6
        for gene in self:
            env_done = env.step(gene.action)
            if env_done:
                done = True
                break
            elif env_done == "last_step":
                gene.action.last_action(env.lander.rotate,env.lander.v_speed)
...
class Population:
    ...
    def roulette_wheel(self):
        size_graded_retain = int(NONGRADED_RETAIN_PERCENT * self.size())
        ...
        for _ in range(size_graded_retain):
            ...
            chromosome_selection.append(scores[i][1])
    ...
    def test_roulette_wheel(self):
        ...
        for _ in range(size_graded_retain):
            ...
            while not done:
                i = random.randint(0,size_graded_retain)
                random_percent = random.random()
                if scores[i][0] > random_percent:
                    chromosome_selection.append(scores[i][1])
                    done = True
        return chromosome_selection
